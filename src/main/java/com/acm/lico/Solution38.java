...
public class Solution38 {
    /**
     ...
     5 执行结果
     解答成功:
     执行耗时:8 ms,击败了43.53% 的Java用户
     内存消耗:38.8 MB,击败了91.62% 的Java用户
     */
    public String countAndSayNew22(int n) {
        int i = 0;
        String result = "1";
        for(i = 1; i < n; i++){
            ...
            for(int k = 0; k < result.length(); k ++) {
                int count = 1;
                char temp = result.charAt(k);
                while (k + 1 < result.length() && result.charAt(k + 1) == temp) {
                    k ++;
                    count ++;
                }
